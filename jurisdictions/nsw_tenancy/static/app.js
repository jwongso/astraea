...
function _renderDebugPanel(dbg, dbgDone) {
  ...
  panel.innerHTML = `<h4>Retrieval debug &mdash; <em>${strategyLabel}</em></h4>${bars}${stats}`;
...
function _buildAnchorCard(s, anchorMethod) {
  const num = (s.document_id || '').replace('NSWLEG/RTA2010/', '');
...
${noText ? '<div class="ctx-anchor-warn">Warning: anchor section selected but no text extracted - section was not sent to model.</div>' : ''}
...
function _renderContextDebugPanel(ev, container) {
  ...
  if (pl.property_change_triggered) {
    const sections = (pl.forced_sections || []).map(s => s.replace('NSWLEG/RTA2010/', '')).join(', ');
  ...
  if (sr.triggered) {
    ...
    const injected = (sr.forced_sections || []).map(s => s.replace('NSWLEG/RTA2010/', '')).join(', ') || 'none';
    ...
    if (sr.suppressed_sections && sr.suppressed_sections.length) {
      const sup = sr.suppressed_sections.map(s => s.section.replace('NSWLEG/RTA2010/', '')).join(', ');
  ...
  if (anchor.sections && anchor.sections.length) {
    ...
    lbl.textContent = `RTA 2010 anchor - ${anchor.method || 'unknown'} (legislation, not [SN] cited)`;
...
function _renderSharedContextDebugPanel(ev, container) {
  ...
  if (anchor.sections && anchor.sections.length) {
    const lbl = document.createElement('div');
    lbl.className = 'ctx-section-label';
    lbl.textContent = `RTA 2010 anchor - ${anchor.method || 'unknown'} (shared, not [SN] cited)`;
    body.appendChild(lbl);
    anchor.sections.forEach(s => body.appendChild(_buildAnchorCard(s, anchor.method)));
  }
...
const LOADING_MESSAGES = [
  'Searching through NCAT decisions...',
...
const _FORBIDDEN_ANCHOR_TERMS_DISPLAY = [
  'Schedule 1', 'infringement', 'penalty notice',
];
...
function renderVerification(sections) {
  ...
  let html = '<div class="verification-header"><span class="verification-check">&#10003;</span> Verified against current legislation (legislation.nsw.gov.au)</div>';
  sections.forEach(s => {
...
function _colSetSources(strategy, sources, legislation) {
  ...
  if (hasDec) {
    ...
    html += sources.map((s, i) => {
      const label = s.date ? `${s.court_name || 'NCAT'} - ${s.date}` : (s.court_name || 'NCAT');
      const url = (s.url || '').startsWith('https://') ? s.url : '#';
...
function _colSetScores(strategy, dbg) {
  ...
  col.querySelector('.compare-col-scores').innerHTML =
    scores.map((s, i) => {
      const pct = Math.round(s * 100);
      const cls = s >= 0.80 ? 'high' : s >= 0.76 ? 'mid' : 'low';
      return `<div class="compare-score-row"><span class="compare-score-label">S${i+1}</span><div class="compare-score-bar-wrap"><div class="debug-score-bar ${cls}" style="width:${pct}%"></div></div><span class="compare-score-val">${s.toFixed(4)}</span></div>`;
...
// ---- Disclaimer modal ----
const _AGREED_KEY = 'nswth_agreed_v1';
